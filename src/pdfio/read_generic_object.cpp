...
std::map<int, std::function<void(std::istream &, pdfio::GenericObject &)>> 
   &GetReadHandlers()
{
   static std::map<int, std::function<void(std::istream &, 
      pdfio::GenericObject &)>> readHandlers;
   return readHandlers;
}
...
void RegisterReadHandler(int type, 
   std::function<void(std::istream &, GenericObject &)> handler)
{
