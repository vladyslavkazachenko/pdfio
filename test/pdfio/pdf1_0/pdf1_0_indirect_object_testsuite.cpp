...
TEST(IndirectObjectTestSuite, defaultCtor)
{
	...
	EXPECT_TRUE(indirectObject.objectNumber() == 0ll);
	EXPECT_TRUE(indirectObject.generationNumber() == 0ll);
}

TEST(IndirectObjectTestSuite, assignment)
{
	pdf1_0::IndirectObject indirectObject;
	indirectObject.objectNumber() = 10ll;
	indirectObject.generationNumber() = 1ll;
	EXPECT_TRUE(indirectObject.objectNumber() == 10ll);
	EXPECT_TRUE(indirectObject.generationNumber() == 1ll);
}

TEST(IndirectObjectTestSuite, setInteger)
{
	pdf1_0::IndirectObject indirectObject;
	indirectObject.set(pdf1_0::Integer(23ll));
	EXPECT_TRUE(indirectObject.get<pdf1_0::Integer>() == 23ll);
...
{
	pdf1_0::IndirectObject indirectObject;
	indirectObject.set(pdf1_0::IndirectReference(23ll, 2l));
	EXPECT_TRUE(indirectObject.get<pdf1_0::IndirectReference>().objectNumber() == 23ll);
	EXPECT_TRUE(indirectObject.get<pdf1_0::IndirectReference>().generationNumber() == 2ll);
...
TEST(IndirectObjectTestSuite, setDictionary)
{
	...
	dictionary.insert(pdf1_0::Name("key1"), pdf1_0::Integer(56ll));
	dictionary.insert(pdf1_0::Name("key2"), pdf1_0::Name("value2"));
	indirectObject.set(dictionary);
	EXPECT_TRUE(indirectObject.get<pdf1_0::Dictionary>().get<pdf1_0::Integer>(pdf1_0::Name("key1")) == 56ll);
