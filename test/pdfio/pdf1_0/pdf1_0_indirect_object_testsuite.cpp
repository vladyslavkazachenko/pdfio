...
TEST(IndirectObjectTestSuite, indirectReference)
{
   ...
   EXPECT_EQ(23, 
      indirectObject.get<pdf1_0::IndirectReference>().objectNumber());
   EXPECT_EQ(2, 
      indirectObject.get<pdf1_0::IndirectReference>().generationNumber());
   EXPECT_EQ(
      static_cast<int>(pdf1_0::GenericObjectType::kIndirectReference), 
      indirectObject.get().typeId_);
}
...
TEST(IndirectObjectTestSuite, setDictionary)
{
   ...
   EXPECT_EQ(56, 
      indirectObject.get<pdf1_0::Dictionary>().get<pdf1_0::Integer>(
      pdf1_0::Name("key1")));
   EXPECT_TRUE(
      indirectObject.get<pdf1_0::Dictionary>().get<pdf1_0::Name>(
      pdf1_0::Name("key2")) == "value2");
   EXPECT_EQ(static_cast<int>(pdf1_0::GenericObjectType::kDictionary), 
      indirectObject.get().typeId_);
}
